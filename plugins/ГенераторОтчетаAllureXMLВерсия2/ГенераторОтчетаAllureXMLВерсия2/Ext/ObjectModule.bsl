...
Функция СоздатьОписаниеШага(Знач РезультатШага, Знач КоллекцияРодителей = Неопределено)
	...
	СтруктураМеток = ПолучитьМетки(КоллекцияРодителей, Родитель);
	...
	ОписаниеШага.historyId = СтрШаблон_("%1.%2.%3.%4", ИмяФичи, ИмяСтори, РезультатШага.Представление, ИмяПакета);

	СписокМеток = ОписаниеШага.labels;
	ДобавитьМетку(СписокМеток, "host", ИмяХоста);
	ДобавитьМетку(СписокМеток, "package", ИмяПакета);
	Для Каждого Метка Из СтруктураМеток Цикл
		ДобавитьМетку(СписокМеток, Метка.Ключ , Метка.Значение);
	КонецЦикла;
...
Функция ПолучитьМетки(Знач КоллекцияРодителей, Знач Родитель)
    
	    Перем Индекс, МассивВложений, МассивМеток, СтруктураМеток;
	    
	    МассивМеток = Новый Массив;
	    МассивМеток.Добавить("story");
	    МассивМеток.Добавить("feature");
	    МассивМеток.Добавить("epic");
	    
	    МассивВложений = Новый Массив;
	    ЗаполнитьМассивВложений(КоллекцияРодителей, МассивВложений, Родитель);
	    СтруктураМеток = Новый Структура;
	    Для Индекс = 0 По МассивВложений.ВГраница() Цикл 
	        СтруктураМеток.Вставить(МассивМеток[Индекс], МассивВложений[Индекс]);
	    КонецЦикла;
	    Возврат СтруктураМеток;
	
	КонецФункции
	
	Процедура ЗаполнитьМассивВложений(КоллекцияРодителей, МассивВложений, Родитель)
	    
	    МассивВложений.Добавить(Родитель.Имя);
	    РодительРодителя = НайтиПоКлючу(КоллекцияРодителей, Родитель.Ключ);
	    Если РодительРодителя <> Неопределено Тогда 
	        ЗаполнитьМассивВложений(КоллекцияРодителей, МассивВложений, РодительРодителя);
	    КонецЕсли;
	КонецПроцедуры	
